...
r = requests.get(f"https://studio-api.prod.suno.com/api/clip/{sample_id}", headers=headers)
data = r.json()
print("Keys in clip response:", list(data.keys()))
for k, v in data.items():
    if "url" in k or "video" in k or "wav" in k or "audio" in k:
        print(f"  {k}: {v}")
