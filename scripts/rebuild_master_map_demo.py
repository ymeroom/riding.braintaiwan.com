...
# Master interactive 19-Day Demo builder with rich visual timeline
...
<head>
    ...
    <title>東京・富士五湖・伊豆・東京灣 19日秋季單車騎旅 ｜ 19天全路線地圖、標高剖面與在地避坑互動 Demo</title>
    <!-- Leaflet CSS & JS -->
    ...
    <style>
        ...
        * {{
        ...
        .tl-dot.start {{ background: #10B981; width: 12px; height: 12px; left: -7px; box-shadow: 0 0 6px #10B981; }}
        .tl-dot.pivot {{ background: #F59E0B; width: 12px; height: 12px; left: -7px; box-shadow: 0 0 8px #F59E0B; }}
        .tl-dot.scenic {{ background: #38BDF8; }}
        .tl-dot.rest {{ background: #A855F7; }}
        .tl-dot.warning {{ background: #EF4444; width: 12px; height: 12px; left: -7px; box-shadow: 0 0 6px #EF4444; }}
        .tl-dot.end {{ background: #EF4444; width: 12px; height: 12px; left: -7px; }}
...
<div class="container">
    ...
    <header>
        <div class="header-title">
            ...
            <p>【全 19 天 OSRM 單車路網幾何 ＋ 日本國土地理院 GSI 1m DEM 實測高程 ＋ 在地避坑時間軸 Demo】</p>
        </div>
        <div class="nav-links">
    ...
    <div class="main-layout">
        ...
        <div class="sidebar">
            ...
            <div class="sidebar-section-title">
                <span>📍 關鍵路口與時間軸 (點擊地圖定位)</span>
                <span style="font-size: 11px; color: #38BDF8; font-weight: normal;">Click to Fly ✈️</span>
            </div>
            ...
            <!-- 在地車友避坑實戰解析 -->
            <div class="tip-callout" id="tipCallout">
                <!-- Rendered via JS -->
            </div>

            <div class="quick-nav-btns">
                <a href="#" id="btnGoogle" target="_blank" class="app-btn">🗺️ Google 地圖導航 ↗</a>
                <a href="#" id="btnNavitime" target="_blank" class="app-btn">🗾 NAVITIME 導航 ↗</a>
                <button id="btnDownloadGPX" class="app-btn gpx-btn">💾 下載當日 GPX 軌跡檔 (Garmin/Wahoo/Komoot)</button>
            </div>
        </div>
    </div>
...
<script>
    // 19 Days Comprehensive Dataset with Rich Timelines and Expert Tips
    const daysData = {json_data_str};

    let currentDayIdx = 0;
    let map, polylineLayer, startMarker, endMarker, hoverMarker, timelineMarkers = [];
    let elevationChart;
    ...
    function selectDay(idx) {{
        ...
        if (d.timeline && d.timeline.length > 0) {{
            d.timeline.forEach((item, i) => {{
                ...
                div.innerHTML = `
                    ...
                    <div class="tl-title">
                        <span>${{item.name}}</span>
                        <span class="tl-km">${{item.km}} km</span>
                    </div>
                    <div class="tl-desc">${{item.desc}}</div>
                `;
                div.onclick = () => flyToTimelinePoint(item);
                timelineList.appendChild(div);
            }});
        }}

        // Update Callout Tip
        document.getElementById('tipCallout').innerHTML = d.expert_tip || "💡 騎行平安，享受美好秋景！";
        ...
        document.getElementById('btnNavitime').href = `https://www.navitime.co.jp/maps/routeSearch?start=%7B%22lat%22%3A${{startPt[1]}}%2C%22lon%22%3A${{startPt[0]}}%7D&goal=%7B%22lat%22%3A${{endPt[1]}}%2C%22lon%22%3A${{endPt[0]}}%7D`;
    ...
    // Fly to Timeline Point on Click
    function flyToTimelinePoint(item) {{
        if (item.coord && map) {{
            map.flyTo([item.coord[1], item.coord[0]], 14, {{
                animate: true,
                duration: 1.0
            }});
            L.popup()
                .setLatLng([item.coord[1], item.coord[0]])
                .setContent(`<b>${{item.name}} (${{item.km}} km)</b><br><small>${{item.desc}}</small>`)
                .openOn(map);
        }}
    }}
    ...
    function updateMapRoute(d) {{
        ...
        timelineMarkers.forEach(m => map.removeLayer(m));
        timelineMarkers = [];
        ...
        // Add visual markers for timeline pivot points
        if (d.timeline) {{
            d.timeline.forEach(item => {{
                if (item.type === 'pivot' || item.type === 'warning') {{
                    const m = L.circleMarker([item.coord[1], item.coord[0]], {{
                        radius: 6,
                        fillColor: item.type === 'warning' ? '#EF4444' : '#F59E0B',
                        color: '#FFFFFF',
                        weight: 2,
                        fillOpacity: 1
                    }}).bindPopup(`<b>${{item.name}} (${{item.km}} km)</b><br>${{item.desc}}`).addTo(map);
                    timelineMarkers.push(m);
                }}
            }});
        }}
...
print("Master 19-Day Interactive Map Demo rebuilt with rich visual timeline and expert tips!")
