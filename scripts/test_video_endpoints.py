import requests
import json
import sys

token = """..MzDvniM3ZczrUc32wSHB12crp81S7pV4eBbQ2xC2JbrbUTPGBu3-Iy5F0n8k-7_x12PrR80iqGjRerrj4NKdzKG8-wbLbavXSu1qmpsFUN_BZ2pLtu6HV-lVMruk2x6g2Dp0jG3Gi-CgKPszgjMS2CfJnaPg-8Bnij6wa3aB2wXS3t1KPvsTXBYbsDhS2VKPZ0CROwrjHYulFQ7KyDim09aXufi6DgLo-7jS4Vmk-g6zVMd62l22nV9yTTIuh42DI0_el0Ct6O4FTYR4F7jTLHF6fPrZNUs3b7zS7GSNe3-gIlAViz6E81Cr52IQZ7z2GYKw-q2FAeJIy-UDq3vZOw""".strip()

headers = {
    "Authorization": f"Bearer {token}",
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36",
    "Content-Type": "application/json",
    "Accept": "application/json, text/plain, */*",
    "Referer": "https://suno.com/",
    "Origin": "https://suno.com"
}

sample_id = "2f5332d6-d190-4d1d-b8d5-08a53a273aa7"

candidates = [
    ("POST", "https://studio-api.prod.suno.com/api/video/", {"clip_id": sample_id}),
    ("POST", f"https://studio-api.prod.suno.com/api/clip/{sample_id}/video", {}),
    ("POST", f"https://studio-api.prod.suno.com/api/clip/{sample_id}/video/", {}),
    ("POST", f"https://studio-api.prod.suno.com/api/clip/{sample_id}/mp4/", {}),
    ("POST", f"https://studio-api.prod.suno.com/api/clip/{sample_id}/wav", {}),
    ("POST", f"https://studio-api.prod.suno.com/api/clip/{sample_id}/wav/", {}),
    ("POST", f"https://studio-api.prod.suno.com/api/audio/wav", {"clip_id": sample_id}),
    ("POST", f"https://studio-api.prod.suno.com/api/audio/video", {"clip_id": sample_id}),
    ("GET", f"https://studio-api.prod.suno.com/api/clip/{sample_id}/download_url?type=video"),
    ("GET", f"https://studio-api.prod.suno.com/api/clip/{sample_id}/download_url?type=wav"),
    ("GET", f"https://studio-api.prod.suno.com/api/clip/{sample_id}/download?type=wav"),
    ("GET", f"https://studio-api.prod.suno.com/api/clip/{sample_id}/download?type=video"),
]

for method, url, *body in candidates:
    try:
        b = body[0] if body else None
        if method == "POST":
            r = requests.post(url, headers=headers, json=b, timeout=6)
        else:
            r = requests.get(url, headers=headers, timeout=6)
        print(f"{method} {url} -> Status {r.status_code}")
        if r.status_code != 404:
            print(f"   Response: {r.text[:200]}")
    except Exception as e:
        print(f"Err {url}: {e}")
